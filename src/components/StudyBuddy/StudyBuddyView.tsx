...
import { Brain, Send, Sparkles, BookOpen, FlaskConical, Calculator, PenTool, Lightbulb, RefreshCw, Copy, CheckCircle2, ChevronDown } from "lucide-react";
...
const SUBJECTS = [
  ...
  { id: "history", label: "History & Social Studies", icon: "???", color: "from-amber-500 to-orange-500" },
...
export const StudyBuddyView: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      role: "assistant",
      content: `Hi ${currentUser.name.split(" ")[0]}! ?? I am your AI Campus Study Buddy, powered by Gemini.\n\nI can help you with:\n� ?? **Mathematics** � equations, proofs, and problem-solving\n� ?? **Science** � physics, chemistry, biology explanations\n� ?? **English & Literature** � essay writing, analysis, grammar\n� ??? **History** � events, timelines, and analysis\n� ?? **Coding** � algorithms, debugging, and concepts\n� ?? **General Exam Prep** � revision strategies and practice questions\n\nWhat subject are you studying today? Pick one below or just ask your question!`,
  ...
  const renderContent = (text: string) => {
    return text.split("\n").map((line, i) => {
      ...
      if (line.startsWith("� ") || line.startsWith("- ")) {
        return <p key={i} className="flex gap-1.5"><span className="text-blue-400 mt-0.5">�</span><span>{line.slice(2).replace(/\*\*(.*?)\*\*/g, "$1")}</span></p>;
      }
      if (/^\d+\./.test(line)) {
        return <p key={i} className="flex gap-1.5"><span className="text-blue-400 font-bold">{line.match(/^\d+/)?.[0]}.</span><span>{line.replace(/^\d+\.\s*/, "").replace(/\*\*(.*?)\*\*/g, "$1")}</span></p>;
      }
      if (line.startsWith("###")) return <h4 key={i} className="font-black text-sm mt-2">{line.slice(3).trim()}</h4>;
      if (line.startsWith("##")) return <h3 key={i} className="font-black text-base mt-2">{line.slice(2).trim()}</h3>;
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] min-h-[550px] bg-white rounded-3xl border border-neutral-200/80 shadow-xs overflow-hidden">
      {/* Header */}
      <div className="flex-shrink-0 bg-gradient-to-r from-violet-600 via-purple-600 to-blue-600 text-white px-5 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-white/70">Powered by Google Gemini � Campus Connect</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[9px] bg-white/20 backdrop-blur-sm px-2 py-1 rounded-full font-bold uppercase tracking-wider flex items-center gap-1">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-neutral-50/50">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
            {msg.role === "assistant" && (
              <div className="w-8 h-8 rounded-2xl bg-gradient-to-br from-violet-500 to-blue-500 flex items-center justify-center flex-shrink-0">
                <Sparkles className="w-4 h-4 text-white" />
              </div>
            )}
            {msg.role === "user" && (
              <img src={currentUser.avatar} alt="" className="w-8 h-8 rounded-full object-cover flex-shrink-0 ring-2 ring-neutral-200" />
            )}
            <div className={`flex-1 max-w-2xl ${msg.role === "user" ? "flex justify-end" : ""}`}>
              <div className={`rounded-2xl px-4 py-3 text-xs leading-relaxed space-y-1 ${
                msg.role === "user"
              ...
              <div className={`flex items-center gap-2 mt-1 px-1 ${msg.role === "user" ? "justify-end" : ""}`}>
                ...
                {msg.role === "assistant" && (
                  <button onClick={() => copyToClipboard(msg.id, msg.content)}
                    className="text-neutral-400 hover:text-neutral-600 transition-colors">
                    {copiedId === msg.id ? <CheckCircle2 className="w-3 h-3 text-emerald-500" /> : <Copy className="w-3 h-3" />}
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
          {QUICK_PROMPTS.map((prompt, i) => (
            <button key={i} onClick={() => sendMessage(prompt)}
              className="flex-shrink-0 text-[10px] font-medium px-2.5 py-1.5 rounded-xl bg-violet-50 text-violet-700 border border-violet-100 hover:bg-violet-100 transition-colors whitespace-nowrap">
              {prompt}
            </button>
          ))}
        </div>
      ...
      {/* Input */}
      <div className="flex-shrink-0 p-3.5 bg-white border-t border-neutral-200">
        <form onSubmit={handleSubmit} className="flex gap-2">
          ...
          <button type="submit" disabled={!input.trim() || isLoading}
            className="p-2.5 bg-gradient-to-br from-violet-600 to-blue-600 disabled:opacity-40 text-white rounded-2xl transition-all hover:brightness-110 active:scale-95 shadow-lg shadow-violet-500/20">
            <Send className="w-4 h-4" />
          </button>
        </form>
        <p className="text-[9px] text-neutral-400 text-center mt-2">AI Study Buddy can make mistakes. Always verify important exam content with your teacher.</p>
      </div>
    </div>
  );
};
